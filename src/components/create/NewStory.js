...
function NewStory({ handleChange, plantNickname, acquiredOn, commonName, latinName, photo, caption, collectionID, createStorySubmit, userCollections}) {
  return (
    ...
    <label>
      ...
      <TextField value={acquiredOn} label="aqcuired on" name="acquiredOn" type="date" />
      ...
      <select name={"collectionID"} onChange={handleChange} >
        {userCollections.map((collection)=> <option  value={collection.id} name={collectionID}>{collection.name}</option>)}
      </select>
    </label>
